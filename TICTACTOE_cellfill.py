...
while True:
    ...
    if event.type == MOUSEBUTTONDOWN and pygame.mouse.get_pressed()[0] == 1 and winner[0] == 'N':
        key = 0
        for i in range(1, 10):
    ...
    if winner[0] != 'N':
        if winner[0] == 'D':
            ...
            winsurfaceobj = winfontobj.render(winner[0] + '   W I N S ', True, ORANGE)
        if winner[0] != 'D':
            pygame.draw.rect(gamedisplay, YELLOW, box[winner[1]])
            pygame.draw.rect(gamedisplay, YELLOW, box[winner[2]])
            pygame.draw.rect(gamedisplay, YELLOW, box[winner[3]])
            if winner[0] == 'X':
                xoobj = xofontobj.render('X', True, RED)
            else:
                xoobj = xofontobj.render('O', True, BLUE)
            xorect = xoobj.get_rect()
            xorect.center = box[winner[1]].center
            gamedisplay.blit(xoobj, xorect)
            xorect = xoobj.get_rect()
            xorect.center = box[winner[2]].center
            gamedisplay.blit(xoobj, xorect)
            xorect = xoobj.get_rect()
            xorect.center = box[winner[3]].center
            gamedisplay.blit(xoobj, xorect)
